...
namespace Item
{
    public class ItemData : MonoBehaviour
    {
        ...
        private void RegisterEquipmentItem()
        {
            ...
            Equipment shoulder1 = new Equipment(210, 160, "2002", "shoulder1", "어깨빵", "싸고 효율좋은 어깨빵", "shoulder", equipmentSpriteList[1]);
            shoulder1.UseEvent += (item) => UseItemFunction(sword1);
            shoulder1.TargetSlot = targetSlotList[0];
            equipmentItemList.Add(shoulder1);

            Equipment bow1 = new Equipment(220, 170, "2003", "bow1", "활활타오르는활활", "화살은 별도입니다", "weapon", equipmentSpriteList[2]);
            bow1.UseEvent += (item) => UseItemFunction(bow1);
            bow1.TargetSlot = targetSlotList[1];
            equipmentItemList.Add(bow1);

            // Equipment의 Target Slot을 지정
        ...
        {
            CommonItem iron1 = new CommonItem(1, 5, 20, 10, "3001", "iron1", "철철김철강철", "고급인력이다", "material", commonItemSpriteList[0]);
            iron1.UseEvent += (item) => UseItemFunction(iron1);
            commonItemList.Add(iron1);

            CommonItem paper1 = new CommonItem(1, 5, 30, 15, "3002", "paper1", "컨닝페이퍼", " A+ or F", "material", commonItemSpriteList[1]);
